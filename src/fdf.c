...
int		main(int argc, char **argv)
{
	...
	if ((ft_read_line(argv[1], &var) != -1))
	{
		...
		var.mlx.width *= 0.8;
		var.mlx.height *= 0.6;
		var.mlx.win_ptr = mlx_new_window(var.mlx.mlx_ptr, var.mlx.width, var.mlx.height, "FDF");
		var.mlx.img_ptr = mlx_new_image(var.mlx.mlx_ptr, var.mlx.width, var.mlx.height);
		var.mlx.addr = (int *)mlx_get_data_addr(var.mlx.img_ptr, &var.mlx.bits_per_pixel, &var.mlx.line_length,
				&var.mlx.endian);
		draw_tab(&var);
		mlx_put_image_to_window(var.mlx.mlx_ptr, var.mlx.win_ptr, var.mlx.img_ptr, 0, 0);
		mlx_hook(var.mlx.win_ptr, 17, 0, exit_all, &var);
		mlx_key_hook(var.mlx.win_ptr, key_event, (void *)0);
